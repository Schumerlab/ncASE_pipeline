...
if(@ARGV<10){
    print "perl run_samtools_rnaseq.pl id_list genome1 genome2 read_length mapper path_sam path_bcf path_bedtools gtf save_fils program_path\n"; exit;
}#usage
...
while (my $id = <IN>){
	...
	system("perl $path/merge_files_using_two_columns_sharing_values.pl $aims 0 1 $vcf1 0 1 $overlap1");
	system("perl $path/merge_files_using_two_columns_sharing_values.pl $aims 0 1 $vcf2 0 1 $overlap2");

	#ADD PRINT FILENAMES FOR ncASE_normalize_counts.pl!!!!

	system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap1");
	system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap2");

	my $ase1="$overlap1"."_ASE_counts";
	my $ase2="$overlap2"."_ASE_counts";
	
        my $combined="$indiv"."_combined_ASE";
	system("perl $path/merge_files_using_two_columns_sharing_values.pl $ase1 0 1 $ase2 0 1 $combined");
	...
	system("perl $path/ASE_to_transcripts.pl $gtf1 transcript > $vcf1");
	system("perl $path/ASE_to_transcripts.pl $gtf2 transcript > $vcf2");

	$overlap1="$indiv"."_par1";
        $overlap2="$indiv"."_par2";

	system("perl $path/merge_files_using_two_columns_sharing_values.pl $aims 0 1 $vcf1 10 1 $overlap1");
        system("perl $path/merge_files_using_two_columns_sharing_values.pl $aims 0 1 $vcf2 10 1 $overlap2");

	system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap1 $gtf");
        system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap2 $gtf");
  ...
  # output ASE_counts files for each ref genome alignment
	system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap1");
	system("perl $path/samtools_vcf_to_ASE_counts_v3.pl $overlap2");
	}#gtf not provided
	my $ase1="$overlap1"."_ASE_counts";
