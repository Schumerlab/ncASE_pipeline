...
if(length(args) < 6)
  stop("Need at least one pair of replicate files, read length, method, bias threshold, and output filename")
if(length(args) %% 2 == 1)
  stop("Need two files per replicate")

max_replicate <- (length(args)-4)/2
read_length<-as.numeric(args[length(args)-3])
method<-as.numeric(args[length(args)-2])
bias<-as.numeric(args[length(args)-1])
...
if(method==0){
snps <- subset(snps, snps$species_1_species_1>0 & snps$species_2_species_1>0 & snps$species_1_species_2>0 & snps$species_2_species_2>0)

lower=0.5-bias
upper=0.5+bias

snps <- snps[snps$species_1_species_1/(snps$species_1_species_1 + snps$species_2_species_1) > lower
                   & snps$species_1_species_1/(snps$species_1_species_1 + snps$species_2_species_1) < upper
                   & snps$species_2_species_2/(snps$species_1_species_2 + snps$species_2_species_2) > lower
                   & snps$species_2_species_2/(snps$species_1_species_2 + snps$species_2_species_2) < upper,]
}#no zero counts allowed
...
snps <- snps[(snps$species_1_species_1+0.01)/(snps$species_2_species_1+0.01) > lower
                   & (snps$species_1_species_1+0.01)/(snps$species_2_species_1+0.01) < upper
                   & (snps$species_1_species_2+0.01)/(snps$species_2_species_2+0.01) > lower
                   & (snps$species_1_species_2+0.01)/(snps$species_2_species_2+0.01) < upper,]
...
for(replicate in 2:max_replicate) {
  ...
  if(max_replicate==1){current_replicate=1}
  temp <- replicate_counts[[current_replicate]]$x.Group.1[replicate_counts[[current_replicate]]$x.Group.1 %in% temp]
}
